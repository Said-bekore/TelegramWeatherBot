...
while (true)
{
    ...
    if (jsonResponse.IsSuccessStatusCode && tgUpdates != null)
    {
        foreach (var update in tgUpdates.Result)
        {
            ...
            {
                responseMessage = "Это телеграм бот который отвечает прогнозом погоды в ответ на название города! Чтобы узнать погоду, введите название города";
            }
                ...
                if (!jsonResponse.IsSuccessStatusCode)
                {
                    switch (jsonResponse.StatusCode)
                    {
                        ...
                        case HttpStatusCode.Unauthorized:
                            responseMessage = "Сервис не отвечает, попробуйте снова через несколько минут. ";
                            ...
                            break;
                        default:
                            responseMessage = "Неизвестная ошибка, свяжитесь с разработчиком SMASHEVRYONE@GMAIL.COM";
    ...
    {
        Console.WriteLine(DateTime.Now.ToString(CultureInfo.InvariantCulture) + ": Произошла ошибка с Телеграм");
    }
